...
\lstdefinelanguage{dj}{
  keywords={int, smallint, char, varchar, enum, unsigned, date, year, decimal, insert, delete},
...
\begin{tabu}{|X[1,c,p]| X[1,c]| X[1,p]| X[1, p]|}
\hline
\rowcolor{HeaderColor}
{\bf operator} & {\bf notation} & {\bf meaning} & {\bf primary key} \\
restrict & \lstinline$A \& cond$  & The subset of all elements of \lstinline$A$ that meet condition \lstinline$cond$ & same as \lstinline$A$  \\
exclude & \lstinline$A \\ cond$  & The subset of all elements of \lstinline$A$ except those that meet condition \lstinline$cond$ & same as \lstinline$A$ \\
join & \lstinline$A * B$ & The combination of matching elements of \lstinline$A$ and \lstinline$B$ & union of \lstinline$A$ and \lstinline$B$ \\
\hline
\end{tabu}
