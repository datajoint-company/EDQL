...
\lstset{
    ...
    commentstyle=\color{gray}\itshape\small,
...
\lstdefinelanguage{dj}{
  ...
  morecomment=[l][\color{DarkGray}\itshape\small]{\#},
  stringstyle=\color{Indigo},
  morestring=[b]',
  morestring=[b]"
}
...
The simplified definition combines two jobs in one step:  add the referencing attributes to the table definition (if not already added) and create the foreign key constraints.  These definition are not only more terse but are less error-prone since changes in the definitions of the referenced table would automatically and correctly propagate to the referencing table. 
...
GpaBySex = U(sex).aggr(GPA, avg_gpa: avg(gpa))
\end{lstlisting}
...
s2 = Student.proj(date_of_birth, 
    other: student_id) * 
    U(date_of_birth)

SharedBday = Student * s2 & 
    student_id < other
\end{lstlisting}
...
We conceived and began work on DataJoint in Andreas S.\ Tolias' lab in the Neuroscience Department at Baylor College of Medicine in the fall of 2009. 
...
Jacob Reimer and Emmanouil Froudarakis became early adopters within the Andreas Tolias Lab and propelled development.
Outside the Tolias lab, the first labs to adopt DataJoint (approx.\ 2010) were the labs of Athanassios G.\ Siapas at CalTech, Laura Busse and Steffen Katzner at University of T\"ubingen.
