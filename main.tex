...
The relational model is abstract and semantically unconstrained, providing few guidelines for translating given problems into database schemas or for forming valid queries. 
...
When a schema does not meet the normal forms, it is considered \emph{unnormalized}. 
Redesigning schemas to meet these tests is called \emph{normalization}.
...
The overall lack of semantic constraints in the relational data model allows a diversity of incompatible approaches to schema design and data queries. 
...
The central concept in ERM is that of an {\em entity set}: an unordered collection of identifiable items in the modeled world that share the same set of attributes, are distinguished from each other by the same \emph{primary key}, and can participate in the same types of relationships with other entity sets. 
In ERM, all base relations are either entity sets or \emph{relationship sets} between entity sets.

ERM diagramming notation depicts entity sets and relationship sets, turning ERM into an effective tool for \emph{conceptual modeling} and communication between database designers and customers or managers.

By prescribing how to map 
